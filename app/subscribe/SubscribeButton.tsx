...
export default function SubscribeButton() {
  ...
  useEffect(() => {
    let cancelled = false;
    if (!planId) { setErr('Plan ID missing'); return; }
    ...
    loadPayPalSdk()
      .then((paypal) => {
        ...
        try {
          ...
          try { btn.render(ref.current); } catch (e: any) {
  ...
  if (err) return <div className="rounded-xl border border-red-800 p-4 text-sm text-red-600">{err}</div>;
  return <div ref={ref}>{!ready && <div className="text-sm text-gray-500">Loading…</div>}</div>;
}